...
{
    public class SiteMapsGenerator
    {
        private readonly XNamespace NS = "http://www.sitemaps.org/schemas/sitemap/0.9";

        private List<SitemapUrl> _urls;

        public SiteMapsGenerator()
        {
            _urls = new List<SitemapUrl>();
        }

        public void AddUrl(string url, string modified = null, ChangeFrequency? changeFrequency = null, double? priority = null)
        {
        ...
        private XElement CreateItemElement(SitemapUrl url)
        {
            ...
            {
                itemElement.Add(new XElement(NS + "lastmod", url.Modified));
            }
